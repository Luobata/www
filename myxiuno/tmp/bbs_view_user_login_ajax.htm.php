...
<form action="http://localhost/myxiuno/?user-login-ajax-1.htm" method="post" id="login_form">
	...
	<dl>
		...
		<dd><input type="text" name="email" id="login_email" value="<?php echo isset($user['email']) ? $user['email'] : '';?>" tabindex="10" style="width: 250px;" /> <a href="http://localhost/myxiuno/?user-create-ajax-1.htm" onclick="return false;" class="ajaxdialog" id="login_create_link">注册</a></dd>
		...
		<dd>
			...
			<?php if($conf['resetpw_on']) { ?><a href="http://localhost/myxiuno/?user-resetpw.htm" target="_blank">找回密码</a><?php } ?>
		</dd>
	...
	$('#login_submit_ajax').click(function() {
		...
		$.post($('#login_form').attr('action'), postdata,  function(s){
			...
			if(!json.user) {
				...
				$('#user').html('<a href="http://localhost/myxiuno/?my-profile.htm">' + user.username + '</a> '+adminadd);
