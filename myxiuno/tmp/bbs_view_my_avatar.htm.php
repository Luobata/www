	...
	<div class="right">
		...
		<div class="bg1 border shadow">
			<div style="width: 400px; padding: 8px;">
				...
				<div>
					<span class="avatar_huge border" style="<?php if($_user['avatar_huge']) { ?>background-image: url(<?php echo isset($_user['avatar_huge']) ? $_user['avatar_huge'] : '';?>);<?php } ?>" id="avatar_huge"></span>
				</div>
				<div class="submit">
					<a type="button" class="button smallblue" id="avatar_button" value="修改头像" href="javascript:void(0)" role="button"><span>修改头像</span></a>
...
<script type="text/javascript" src="http://localhost/myxiuno/view/js/swfupload/swfupload.js"></script>
<script type="text/javascript" src="http://localhost/myxiuno/view/js/clipimg/clipimg.js"></script>
<script type="text/javascript">
$('#avatar_huge').clipimg({
	postparams:null, 
	buttonid: 'avatar_button', 
	uploadurl: 'http://localhost/myxiuno/?user-uploadavatar-<?php echo isset($conf['cookie_pre']) ? $conf['cookie_pre'] : '';?>sid-<?php echo isset($_sid) ? $_sid : '';?>-<?php echo isset($conf['cookie_pre']) ? $conf['cookie_pre'] : '';?>auth-<?php echo isset($_auth) ? $_auth : '';?>-ajax-1.htm',
	clipurl: 'http://localhost/myxiuno/?user-clipavatar-ajax-1.htm',
	baseurl: 'http://localhost/myxiuno/view/js/clipimg/',
	success_recall: function(imgurl, pos) {
		imgurl = imgurl.replace(/_huge\.gif/ig, "_big.gif");
		$('#notice').html('头像修改成功！').show().delay(3000).fadeOut('fast');
		$('#avatar_menu').css('background-image', "url("+imgurl+")");
	}
});
</script>
