...
class admin_control extends common_control {
	...
	function __construct(&$conf) {
		...
		if($login) {
			...
			$url = "http://localhost/myxiuno/admin/?index-login.htm";
	...
	protected function check_admin_group() {
		if($this->_group['groupid'] != 1) {
			$this->message('对不起，您不是管理员，无权访问。', 0, 'http://localhost/myxiuno/admin/?index-login.htm');
		}
	}
	...
	protected function check_mod_group() {
		if($this->_group['groupid'] == 0 || $this->_group['groupid']  > 5) {
			...
			$this->message('对不起，您所在的用户组不是管理组，无权访问。', 0, $this->_group['groupid'] == 0 ? '../?user-login.htm' : 'http://localhost/myxiuno/admin/?index-login.htm');
		}
	}
