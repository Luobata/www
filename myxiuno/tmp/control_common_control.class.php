...
class common_control extends base_control {
	 ...
	 * 用  法：
		...
		$this->message('提交成功！', TRUE, 'http://localhost/myxiuno/?forum-index-123.htm');
	...
	protected function check_forum_access($forum, $action = 'post', &$message) {
		...
		if($access['allow'.$action]) {
			...
			$loginadd = empty($this->_user['uid']) ? ' <a href="http://localhost/myxiuno/?user-login-ajax-1.htm" class="ajaxdialog" onclick="return false" rel="nofollow">点击登录</a>' : '';
