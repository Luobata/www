...
!defined('FRAMEWORK_PATH') && exit('FRAMEWORK_PATH not defined.');
	include 'C:/Users/yaogou/Documents/SoftWare/wamp/wamp/www/myxiuno/tmp/control_admin_control.class.php';
...
class forum_control extends admin_control {
	...
	public function on_merge() {
		...
		if($fid1 && $fid2) {
			...
			$this->message('合并完毕！', 1, 'http://localhost/myxiuno/admin/?forum-merge.htm');
		}
	...
	public function on_delete() {
		...
		$forum = $this->forum->read($fid);
		if(empty($forum)) {
			$this->message('版块已经被删除。', 1, 'http://localhost/myxiuno/admin/?forum-list.htm');
		}
		...
		if($starttid >= $threads) {
			...
			$this->runtime->delete('typearr');
			$this->mcache->clear('forum', $fid);
			$this->message("删除 fid: $fid 完毕 ...", 1, "http://localhost/myxiuno/admin/?forum-delete-fid-$fid.htm");
		} else {
			// 分批删除主题。
			...
			$starttid += $limit;
			$this->message("删除 fid: $fid, tid: $starttid ...", 1, "http://localhost/myxiuno/admin/?forum-delete-fid-$fid-threads-$threads-starttid-$starttid.htm");
		}
		

		
		// 删除首页的缓存
		$this->message('删除完毕', 1, 'http://localhost/myxiuno/admin/?forum-list.htm');
	}
